...
//        query.addPara(null, "button-list").addButton("submit").setValue(T_go);
     ...
     *
     * @hattori This method is basically in search-filters' div, removed/commented out from addBody.
     *
     */
	...
	if(fqs.length > 0 || filterFields.size() > 0){
            List secondarySearchList = options.addList("secondary-search");
            secondarySearchList.setHead(T_filter_label);
	  ...
	  if(filterFields.size() > 0){
	      ...
	      //For each field found (at least one) add options
	      for (DiscoverySearchFilter searchFilter : filterFields) {
		  select.addOption(searchFilter.getIndexFieldName(), message("xmlui.ArtifactBrowser.SimpleSearch.filter.dc." + searchFilter.getIndexFieldName()));
	      }
	      //Add a box so we can search for our value
	      filterComp.addText("filter").setSize(30);
