...
public class SidebarCommunityList extends AbstractDSpaceTransformer implements CacheableProcessingComponent {
	...
	List browse = null;	
	browse = options.addList("community-list");
	...
	browse.setHead(T_FILTER_HEAD);
		...
		if (thiskey.equals("location")){
		    List list = browse.addList(thiskey);
		    java.util.List<DiscoverResult.FacetResult> onekey = values.get(thiskey);
		    
		    Iterator<DiscoverResult.FacetResult> iter = onekey.iterator();
		    while(iter.hasNext()){
			facetvalue = iter.next();
			String displayedValue = facetvalue.getDisplayedValue();
			String filterQuery = facetvalue.getAsFilterQuery();
			
			if (fqs.contains(filterQuery)) {
			    list.addItem(Math.random() + "", "selected").addContent(displayedValue + " (" + facetvalue.getCount() + ")");
			} else {
			    String paramsQuery = retrieveParameters(request);
			    list.addItem().addXref(contextPath + (dso == null ? "" : "/handle/" + dso.getHandle()) + "/discover?" 
						   + paramsQuery + "fq=" + URLEncoder.encode(filterQuery, "UTF-8"), displayedValue 
						   + " (" + facetvalue.getCount() + ")");
			}
		    }
		}
